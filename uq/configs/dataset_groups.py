...
def toy_config(config):
    ...
    sizes = [2**i for i in range(8, 14, 2)]
    names = [f'{prefix}_{size}' for prefix in prefixes for size in sizes]
    args = dict(
        batch_size=512,
        train_inter_val_calib_test_split_ratio=[0.02, 0, 0.02, 0.02, 0.94],
        num_workers=0,
        pin_memory=False,
    )
    datamodule_config = dict(
