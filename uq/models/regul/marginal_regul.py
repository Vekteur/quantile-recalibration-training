...
def sample_spacing_entropy_estimation_from_sorted_pit(sorted_pit, spacing=64):
    """
    Compute entropy using sample spacing entropy estimation.
    """
...
def sample_spacing_entropy_estimation(pit, spacing=64, neural_sort=True, s=1., divergence='l2'):
    # Change the spacing for extreme batch sizes
    N = pit.shape[0]
    if N // 2 == 0:
        return torch.tensor(0, dtype=pit.dtype, device=pit.device)
    elif spacing > N // 2:
        spacing = int(math.sqrt(N))

    if neural_sort:
        #sorted_pit = compute_neural_sort(pit, tau=s)
        sorted_pit = soft_sort(pit[None, :], regularization_strength=s, regularization=divergence)[0]
    else:
        sorted_pit = torch.sort(pit)[0]
    # print(sorted_pit[:5])
    entropy = sample_spacing_entropy_estimation_from_sorted_pit(sorted_pit, spacing)
    # print(neg_entropy)
    return entropy
...
def kde_mc_entropy_estimation(kde_pit, mc_pit, b, base_module=None):
    assert kde_pit.ndim == 1 and mc_pit.ndim == 1, (kde_pit.shape, mc_pit.shape)
    cdf = SmoothEmpiricalCDF(kde_pit, b, base_module=base_module)
