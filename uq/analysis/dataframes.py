...
from uq.train import instantiate, load_datamodule
...
def build_test_metric_accessor(metric):
    def impl(df):
        best_iter = df.metrics['best_iter']
        epoch_values = df.metrics['per_epoch'][metric]
        try:
            return epoch_values[best_iter]
        except KeyError:
...
def get_hparams(df):
    ...
    if type(hparams) == pd.Series:
        return set()
    hparams.remove('inhoc_grid')
    hparams.remove('posthoc_grid')
    return hparams
...
def set_test_metrics_columns(df, test_metrics=None, add_infos=True):
    ...
    for metrics in df.metrics:
        time_metrics.update(metric for metric in metrics if type(metric) == str and metric.endswith('_time'))
    general_metrics = [
        'best_iter',
        'best_score',
        *time_metrics,
        ...
        for metric in general_metrics:
            def accessor(d):
                if metric in d:
                    return d[metric]
                return np.nan
            df[metric] = df.metrics.map(accessor)
...
def make_test_df(
    df,
    config,
    groupby=['dataset_group', 'dataset'],
    average_mode=None,
):
    """
    Transform the dataframe to a dataframe with columns
    ('dataset_group', 'dataset', 'run_id') + all_test_metrics + all_hparams.
...
def make_test_df_for_tuning(df=None, config=None, groupby=None, average_mode=None):
    ...
    if groupby is None:
        groupby = ['dataset_group', 'dataset'] + list(get_hparams(df))
...
def separate_posthoc(df):
    ...
    df['full_model'] = df.apply(
        lambda d: d['regul'] + (' posthoc' if d['posthoc'] else ''),
        axis='columns',
    )
...
def compute_duplication_df(config):
    ...
    for dataset_group, dataset_group_config in config.dataset_groups.items():
        ...
        for dataset in dataset_group_config.names:
            ...
            data['Nb points'].append(x.shape[0])
            data['Nb of features'].append(x.shape[1])
...
def build_grouped_comparison_df(
    ...
    """
    The returned dataframe is grouped by join_by + columns_to_keep - ['run_id']
    """
    join_by = ['dataset_group', 'dataset', 'run_id', 'metric'] + join_by
    columns_to_keep = list(set(df.index.names) - set(join_by))
    ...
    merged = compared.merge(baseline, how='inner', on=join_by, validate='many_to_one')
    ...
    if group:
        ...
        return merged.reset_index().set_index(groupby)
